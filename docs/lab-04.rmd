...
countyarea = step2function(pip_counties, "Counties") %>% 
  dplyr::select(c(area, type, mean, sd, totalArea))
...
squarearea = step2function(pip_counties_grid, "CountiesSquareGrid") %>% 
  dplyr::select(c(area, type, mean, sd, totalArea))
...
tess_summary = bind_rows(countyarea, squarearea, hexagonarea, triangulatedarea, voroniarea) %>% 
  dplyr::select(type, area, mean, sd, totalArea)

knitr::kable(tess_summary,
             caption = "Tess Summary",
             col.names = c("type", "area", "mean", "sd", "totalArea"),
